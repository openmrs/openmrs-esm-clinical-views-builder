...
const InteractiveBuilder = ({ schema, onSchemaChange }: InteractiveBuilderProps) => {
  ...
  const handleEditTabDefinitionModal = (submenuDetails, tabDefinition, configurationKey) => {
    setEditingTab({ tabDefinition, submenuDetails, configurationKey });
  };

  const handleSaveTabDefinition = (updatedTabDefinition) => {
    const updatedSchema = { ...schema };
    const { submenuDetails, configurationKey } = editingTab;

    const submenuConfig =
      updatedSchema['@openmrs/esm-patient-chart-app'].extensionSlots[submenuDetails.slot].configure[configurationKey];

    submenuConfig.tabDefinitions = submenuConfig.tabDefinitions.map((tab) =>
      tab.id === updatedTabDefinition.id ? updatedTabDefinition : tab,
    );
  ...
  return (
    <div className={styles.interactiveBuilderContainer}>
      ...
      {schema && (
        <div>
          {editingTab && (
            <EditTabDefinitionModal
              tabDefinition={editingTab.tabDefinition}
              onSave={handleSaveTabDefinition}
                ...
                <Accordion key={submenuKey}>
                  <AccordionItem title={submenuDetails?.title}>
                    ...
                    {subMenuSlotDetails?.configure?.[configurationKey]?.tabDefinitions?.map((tabDefinition) => (
                      <Tile className={styles.tileContainer} key={tabDefinition.id}>
                        <div className={styles.editStatusIcon}>
                          <Button
                            ...
                            onClick={() =>
                              handleEditTabDefinitionModal(submenuDetails, tabDefinition, configurationKey)
                            }
                          />
                          <Button
                            size="md"
